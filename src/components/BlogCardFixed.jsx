...
const BlogCard = ({ post, onReadMore, linkTo, priority = false }) => {
  ...
  return (
    <article className="post-card h-full flex flex-col rounded-2xl border border-neutral-200/60 bg-white shadow-sm hover:shadow-md transition-shadow w-full max-w-full overflow-hidden" itemScope itemType="https://schema.org/BlogPosting">
      ...
      <div className="flex-1 p-6 sm:p-8 flex flex-col min-w-0 max-w-full">
        ...
        <h3 className="card-title cat-title text-brand-dark mb-3 max-w-full leading-snug text-lg sm:text-xl md:text-2xl clamp-3 md:clamp-4 lg:clamp-5 text-balance whitespace-normal break-words hyphens-none" itemProp="headline">
          {linkTo ? (
            <NormalizedLink to={linkTo} aria-label={title} className="hover:underline focus:underline">
              {title}
            </NormalizedLink>
          ) : (
            title
